...
if (isset($_POST['add_room'])) {
    ...
    if (!$error) {
        $sql = "SELECT * FROM  UniSys_HIM_Rooms WHERE  code='$code' ";
        $res = mysqli_query($mysqli, $sql);
        if (mysqli_num_rows($res) > 0) {
            $row = mysqli_fetch_assoc($res);
            if ($code == $row['code']) {
                $err =  "Room Code Number Already Exists";
            }
        } else {
            $id = $_POST['id'];
            $code = $_POST['code'];
            ...
            $status  = $_POST['status'];
            $type = $_POST['type'];
            $query = "INSERT INTO UniSys_HIM_Rooms (id, code, hostel_name, hostel_code, status, type) VALUES (?,?,?,?,?,?)";
            $stmt = $mysqli->prepare($query);
            $rc = $stmt->bind_param('ssssss', $id, $code, $name, $h_code, $status, $type);
            $stmt->execute();
            if ($stmt) {
                $success = "Added" && header("refresh:1; url=unisys_him_add_room.php?h_code=$h_code&name=$name");
            } else {
